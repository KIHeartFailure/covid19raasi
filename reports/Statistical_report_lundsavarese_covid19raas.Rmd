...
The median age (IQR) is `r pop %>% summarise(med = median(scb_age),
                                             q1 = quantile(scb_age, probs = 0.25),
                                             q3 = quantile(scb_age, probs = 0.75)) %>%
...
with the original variables using sandwich variance estimators 
...
in the RASi group with `r p[2]*100`% and in the no RASi group with `r p[1]*100`%. 
Interpretation: if there is an unmeasured confounder with an association 
with covid as `r oruc[1]` (harmful), for example eGFR, and eGFR is present 
in the RASi group with `r p[2]*100`% and in the no RASi group with `r p[1]*100`%, the effect of RASi 
would be non-significant. If you up the OR to `r oruc[2]` for the unmeasured confounder 
you have a statistically significant harmful effect and if you up the OR to `r oruc[3]` 
