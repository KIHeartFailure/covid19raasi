...
out19 <- ormy(
  ...
  modvarsmy = c("sos_ddr_nc_rasi", "sos_ddr_nc_mra", modvarsns_nc),
  data = pop %>% filter(pop_nc == "Yes"),
  modname = "Negative control - Bleed",
  weightsmy = c("weight_rasi_nc", "weight_mra_nc")
)
out110 <- ormy(
...
outall <- cbind(
  outrasi <- rbind(
    ...
    out18[["sos_ddr_rasi"]],
    out19[["sos_ddr_nc_rasi"]], out110[["sos_ddr_nc_rasi"]], out111[["sos_ddr_nc_rasi"]]
  ),
  outmra <- rbind(
    out11[["sos_ddr_mra"]], out13[["sos_ddr_mra"]], out14[["sos_ddr_mra"]],
    out15[["sos_ddr_mra"]], out16[["sos_ddr_mra"]], out17[["sos_ddr_mra"]],
    out18[["sos_ddr_mra"]],
    out19[["sos_ddr_nc_mra"]], out110[["sos_ddr_nc_mra"]], out111[["sos_ddr_nc_mra"]]
  )
)
...
outforestor <- outrasi %>%
  ...
  mutate(modnameforforest = case_when(
    modname == "All" ~ "All patients",
    modname == "HF" ~ "Patients with\nHF",
    modname == "Hypertension" ~ "Patients with\nhypertension",
    modname == "Kidney disease" ~ "Patients with\nkidney disease",
    modname == "IHD" ~ "Patients with\nIHD",
    modname == "Diabetes" ~ "Patients with\ndiabetes",
    modname == "Stockholm" ~ "Patients in\nStockholm Region"
  ))
...
axis(2,
  at = 1:nrow(outforestor), labels = rev(outforestor$modnameforforest),
